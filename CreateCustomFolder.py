...
def create_folder(base_dir: str, selections: list[str]) -> str:
    folder_name = "_".join(selections)
...
if OPTIONS:
    ...
    selections = {}
    cols = st.columns(1)  # single column, but easy to change
    ...
    if st.button("Create Folder"):
        if "" in selections.values():
            st.warning("Please select an option for all fields.")
        else:
            try:
                path_created = create_folder(out_dir, list(selections.values()))
